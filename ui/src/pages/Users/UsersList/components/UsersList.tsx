...
import { Chip, Typography, FormLabel, Avatar } from "@mui/material";
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <Chip variant="outlined" className="userAvator" size="medium" label={`${tableMeta?.allData?.firstname} ${tableMeta?.allData?.lastname}`} avatar={<Avatar src={tableMeta.rowData[1]} />} />
            );
          },
        },
      },
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1]) {
                return `Start Date: ${v[0].toLocaleDateString("en-GB")}, End Date: ${v[1].toLocaleDateString("en-GB")}`;
              } else if (v[0]) {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <Typography component={"span"} noWrap={true}>
                <FabButton title="View User Info" style={{ marginRight: "10px" }} icon={<span className="fad fa-eye" />} onClick={() => historyClick(EditUser.getPath({ id: tableMeta.rowData[0] }))} />

                <FabButton title="Delete" style={{ marginRight: "10px" }} color="red" onClick={(e) => openPopover(e, tableMeta.rowData[0])} icon={<span className="fad fa-trash" />} />

                <FabButton title="Block" style={{ marginRight: "10px" }} color="yellow" onClick={(e) => openPopover(e, tableMeta.rowData[0])} icon={<span className="fad fa-ban" />} />
